...
import { createSummitAction, updateSummitAction } from "@/app/(dashboard)/summit/actions";
...
type SummitFormProps = {
  mode: "create" | "edit";
  summitId?: string;
  defaultValues?: SummitFormInput;
};

export function SummitForm({
  mode,
  summitId,
  defaultValues = DEFAULT_SUMMIT_VALUES,
}: SummitFormProps) {
  ...
  const onSubmit: SubmitHandler<SummitFormInput> = async (values) => {
    ...
    const result =
      mode === "edit" && summitId
        ? await updateSummitAction(summitId, values)
        : await createSummitAction(values);
  ...
  const heading = mode === "edit" ? "Edit Summit" : "New Summit";
  const description =
    mode === "edit"
      ? "Update the details of this Gender and Development Summit."
      : "Record a Gender and Development Summit.";
  const saveLabel = mode === "edit" ? "Save Changes" : "Save Summit";
